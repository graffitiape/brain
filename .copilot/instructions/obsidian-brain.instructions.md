---
name: Obsidian Brain
description: Use the shared Obsidian Brain memory for relevant coding, docs, git, and workflow tasks.
applyTo: "**"
---

# Obsidian Brain

Use the user's Obsidian vault as persistent long-term memory for relevant GitHub Copilot work.

Vault path:
- First read `~/.copilot/obsidian-brain-path`.
- If that is missing or invalid, fall back to `~/.codex/obsidian-brain-path`, then `~/.claude/obsidian-brain-path`.

Brain folder:
- Use `<vault>/Brain/`.
- Treat it as the shared multi-model AI brain, not storage for one assistant.

At the start of relevant work:
- Read `<vault>/Brain/_Index.md` as the map of what the brain contains.
